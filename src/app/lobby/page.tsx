...
export default function LobbyPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(roomsRef, (snap) => {
      ...
      snap.forEach((child) => {
        ...
        if (data?.meta?.isPublic === true && data?.meta?.status === "waiting") {
  ...
  async function handleCreateRoom() {
    ...
    try {
      ...
      updates[`rooms/${code}/meta`] = {
        ...
        roomName:
          roomName.trim() ||
          `${(user.displayName || "Host").split(" ")[0]}'s Room`,
        status: "waiting",
        isPublic: isPublic,
        createdAt: Date.now(),
      };
  ...
  async function handleJoinRoom(codeToJoin?: string) {
    ...
    if (code.length !== 6) {
      setJoinError("Enter a valid 6-letter code");
      return;
    }
    setJoining(true);
    setJoinError("");
            ...
            {(["create", "join"] as const).map((tab) => (
              <button
                ...
                style={{
                  ...
                  background:
                    activeTab === tab ? "rgba(212,175,55,0.1)" : "transparent",
              ...
              <div>
                ...
                >
                  <div>
                    ...
                    <div
                      style={{ color: "#5a8ab0", fontSize: 12, marginTop: 2 }}
                    >
                  ...
                  style={{
                    ...
                    backgroundColor:
                      joining || joinCode.length !== 6
                        ...
                        : "transparent",
                    color:
                      joining || joinCode.length !== 6 ? "#5a8ab0" : "#111",
                    fontFamily: "Teko, sans-serif",
                    fontWeight: 700,
                    fontSize: 22,
                    letterSpacing: 2,
            ...
            {[
              {
                ...
                desc: "Create a private room or join a public one",
              },
              {
                icon: "⚙️",
                title: "Setup Auction",
                desc: "Host selects 250+ players for the pool",
              },
              {
                icon: "🔨",
                title: "Bid & Win",
                desc: "Bid in real-time, max ₹100Cr budget",
              },
            ].map((step, i) => (
              <div
                key={i}
              ...
              >
                ...
                <div>
                  ...
                  <div
                    style={{ color: "#5a8ab0", fontSize: 12, lineHeight: 1.5 }}
                  >
                    {step.desc}
                  </div>
                </div>
