  ...
  const ranked = teams
    .map((team) => {
      ...
      return {
        ...
        score: Math.max(
          1,
          Math.min(10, 4.5 + balanceScore * 0.45 + totalSpend / 50),
        ),
        strengths: [
          `Balanced core with ${players.length} players`,
          roleCounts.wicketKeeper > 0
            ? "Has wicketkeeping coverage"
            : "Needs wicketkeeping depth",
        ],
        weaknesses: [
          roleCounts.bowler < 3
            ? "Bowling depth is thin"
            : "Could still add more death-bowling control",
        ],
...
`;
    ...
    return NextResponse.json({
      ...
      analysis: {
        ...
        summary:
          parsed.summary || leaderboard[0]?.verdict || "Leaderboard ready.",
        leaderboard,
      },
    });
  } catch (error: any) {
